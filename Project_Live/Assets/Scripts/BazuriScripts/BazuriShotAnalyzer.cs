...
public  class BazuriShotAnalyzer :MonoBehaviour//バズリショットの評価を行うスクリプト
{
  ...
  public  int Analyzer(Camera camera,LayerMask layer)//被写体の状態に応じてスコアを乗算する
    {
   ...
   float distance=Vector3.Distance(camera.transform.position,obj.transform.position);
    ...
    public List<GameObject> DetectVisibleObjects(Camera camera, LayerMask layer)//カメラの中に映ったオブジェクトを検出する
    {
